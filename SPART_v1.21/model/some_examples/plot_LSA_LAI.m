...
for j=1:length(LAI)
%     plot(wl,TOA_R(:,j),'-','Color',[0.9 0.9 0.9]-0.15*j)
% set(axesHandlesToAllLines(j),'Color',[0.9 0.9 0.9]-0.15*j,'linestyle','--');

% set(axesHandlesToAllLines(j+4),'Color', [0.9 0.9 0.9]-0.15*j);

set(axesHandlesToAllLines(j),'Color',colsx(j,:),'linestyle','--');
set(axesHandlesToAllLines(j+4),'Color', colsx(j,:));
end
key= {'TOC LAI = 0.5','TOC LAI = 1','TOC LAI = 3','TOC LAI = 6',...
    'TOA LAI = 0.5','TOA LAI = 1','TOA LAI = 3','TOA LAI = 6'};
% xlim([400,1100])
% ylim([0,0.5])
